...
import com.alibaba.cobar.client.router.rules.hibernate.AbstractHibernateOrientedRule;
...
public class CobarInternalRouterXmlFactoryBeanTest {
    ...
    public void testAssemblingRulesNormally() throws Exception {
        ...
        List<Set<IRoutingRule<HibernateRoutingFact, List<String>>>> rules = router.getRuleSequences();
    ...
    public void testAssemblingRulesWithMultipleSameConfigurations() throws Exception {
        ...
        for (Set<IRoutingRule<HibernateRoutingFact, List<String>>> set : rules) {
            assertEquals(1, set.size());
            IRoutingRule<HibernateRoutingFact, List<String>> r = set.iterator().next();
            if(r instanceof HibernateClassNameShardingRule || r instanceof HibernateMethodNameShardingRule)
            {
                Map<String, Object> funcMap = ((AbstractHibernateOrientedRule)r).getFunctionMap();
